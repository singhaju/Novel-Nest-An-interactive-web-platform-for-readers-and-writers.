...
- [ ] Clone the repo:
  ```bash
  git clone https://github.com/singhaju/Novel-Nest-An-interactive-web-platform-for-readers-and-writers.git
  ```
- [ ] Check out the submission branch:
  ```bash
  cd Novel-Nest-An-interactive-web-platform-for-readers-and-writers
  git checkout Final
  ```
- [ ] All student work lives in the `code/` directory; the other folders (e.g., `backend/`, `Frontend/`) are legacy drafts.
...
Following this checklist leaves the professor with a fully working copy that mirrors the student's development environment.
